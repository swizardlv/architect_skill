...
3. **基础设施与运行拓扑层 (OM - Operational Model / DM)**：面向运维 SRE 与基础设施架构师，回答“组件跑在哪里，如何扛住真实流量与容灾”的硬核问题。
   - **双层演进**: 必须包含从**逻辑运行模型 (Logical OM)**（抽象执行环境、节点角色、安全网络分区 DMZ/Private/Data）向**物理运行模型 (Physical OM)**（具体机房可用区、物理硬件规格、CIDR 网络、中间件集群与 RTO/RPO 拓扑）的演进。
   - **部署映射 (Deployment Allocation)**: 必须清晰标注物理 CM 构建物部署在哪个 OM 节点集群，界定低延迟共存（Colocation）与安全物理隔离规则。
   - **压力实战检验**: 必须经受“拔电源演练”、“容量与成本测算”及“可观测性探针就绪”三大实战压力测试。
