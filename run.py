...
def generate_sample_assets(fsm: ArchitectureLifecycleFSM) -> None:
    ...
    write_file(
        "01-requirements/01-01-business-drivers.md",
        "# Business Drivers & Goals\n\n## 1. 核心业务驱动力\n- 打造确定性物理围栏与双环架构控制引擎\n- 解决传统 AI 编程助手随意越界破坏架构的痛点\n\n## 2. 投资回报与量化目标\n- 任务执行成功率 >= 95%\n- 架构单向推进零回滚",
    )
    write_file(
        "01-requirements/01-02-functional-requirements.md",
        "# Functional Requirements\n\n## 1. 核心用例规约 (Use Cases)\n- UC-01: 架构生命周期 FSM 确定性推进\n- UC-02: 人机交互审批卡点 (HITL)\n\n## 2. 功能需求规约清单\n### FR-01: 状态机推进与门禁自检\n系统必须提供单向状态推进能力，在推进前自动校验当前阶段的必需资产与语法完整性。\n\n### FR-02: 交互式审批与打回流转\n支持技术评审人签署批准或打回意见。\n\n### FR-03: 自动化代码骨架生成\n自动生成符合六边形架构的物理目录与自动化测试套件。",
    )
    write_file(
        "01-requirements/01-03-non-functional-requirements.md",
    ...
    write_file(
        "01-requirements/01-04-architecture-requirements-checklist.md",
    ...
    write_file(
        "01-requirements/01-05-constraints-and-assumptions.md",
        "# Constraints & Invariants\n\n## 1. 系统核心不变量\n- 状态单向演进，禁止未定义回滚\n- 只读契约目录禁止直接写\n- Domain 层严禁反向依赖外部适配器",
    )

    # 2. 02-architecture-design/ (MODELING 门禁资产)
    write_file(
        "02-architecture-design/02-01-system-overview.md",
...
```
"""
    write_file("02-architecture-design/02-02-architecture-overview-diagram.md", aod_code)
...
```
"""
    write_file("02-architecture-design/02-04-component-model.md", cm_code)
    ...
    dev --> core
"""
    write_file("02-architecture-design/02-03-c4-context.mmd", c4_context_code)

    c4_container_code = """graph TB
    cli["CLI Gateway"] --> fsm["FSM Engine"]
    fsm --> db[("State Storage")]
"""
    write_file("02-architecture-design/02-05-c4-container-overview.mmd", c4_container_code)
...
```
"""
    write_file("02-architecture-design/02-06-domain-logical-model.md", domain_md_code)

    seq_code = """# Sequence and Dataflow
```mermaid
sequenceDiagram
    autonumber
...
```
"""
    write_file("02-architecture-design/02-07-sequence-and-dataflow.md", seq_code)

    # 3. 03-engineering-and-physics/ (CONTRACTS 门禁资产)
    write_file(
        "03-engineering-and-physics/03-01-operational-model.md",
        "# Operational Model\n\n系统采用多环境容器化部署，支持本地单机验证与云原生 Kubernetes 编排运行。",
    )
    write_file(
        "03-engineering-and-physics/03-02-deployment-architecture.md",
        "# Deployment Architecture\n\n```mermaid\ngraph LR\n Client --> Gateway --> ServicePool\n```",
    )
    write_file(
        "03-engineering-and-physics/03-03-data-architecture.md",
        "# Data Architecture\n\n状态数据通过强一致事务写入关系型库，工件全量沉淀在对象存储或本地仓库。",
    )
    write_file(
        "03-engineering-and-physics/03-04-observability-design.md",
        "# Observability Design\n\n集成 OpenTelemetry 链路追踪，记录所有状态流转与门禁执行日志。",
    )
    write_file(
        "03-engineering-and-physics/03-05-failure-resilience-matrix.md",
        "# Failure Resilience Matrix (FMEA)\n\n| 故障场景 | 影响级别 | 容灾策略 | 恢复时效 |\n| :--- | :--- | :--- | :--- |\n| 进程意外崩溃 | 中 | 从持久化 .state.json 自动恢复 | < 1s |\n| 门禁未达成 | 低 | 确定性拦截并提示修复 | 即时 |",
    )
    write_file(
        "03-engineering-and-physics/03-06-interface-contracts-overview.md",
        "# Interface Contracts Overview\n\n定义了 OpenAPI 3.1 规格的外部 REST 通信接口与内部服务协议。",
    )
    write_file(
        "03-engineering-and-physics/03-08-adrs/adr-index.md",
        "# Architecture Decision Records\n\n- [ADR-001: 确定性状态机外壳](03-08-01-ADR-001-fsm-shell.md)",
    )
    write_file(
        "03-engineering-and-physics/03-08-adrs/03-08-01-ADR-001-fsm-shell.md",
        "# ADR-001: 确定性状态机外壳设计\n\n## 状态\nACCEPTED\n\n## 决定\n采用有限状态机严格控制系统架构推进。",
    )
    ...
    get:
      ...
      responses:
        '200':
          description: OK
"""
    write_file("03-engineering-and-physics/03-07-openapi.yaml", openapi_content)
...
"""
    ...
    write_file("04-delivery-and-organization/04-05-agent-rules.md", agent_rules)
    if not (fsm.repo_root / ".agent-rules.md").exists():
        (fsm.repo_root / ".agent-rules.md").write_text(agent_rules, encoding="utf-8")

    write_file(
        "04-delivery-and-organization/04-01-organization-structure.md",
        "# Organization Structure\n\n开发团队采用纵向微团队编制，分为核心架构师组与领域实施组。",
    )
    write_file(
        "04-delivery-and-organization/04-02-estimation-and-plan.md",
        "# Estimation & Delivery Plan\n\n分为 M0 破冰骨架、M1 核心能力、M2 加固集成 3 个迭代周期。",
    )
    write_file(
        "04-delivery-and-organization/04-03-first-step-poc.md",
    ...
    write_file(
        "04-delivery-and-organization/04-04-walking-skeleton-spec.json",
